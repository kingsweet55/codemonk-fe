...
import { PrivateGrid, P, H3, ToastifyMessage, Badge, A, Button, Card, SVGIcon, AppIcon } from 'components';
...
import * as ICONS from 'containers/App/constants';
import ArchiveBriefModal from './ArchiveBriefModal';
import { ViewDetailBtn, CardEmpty } from './styles';
...
export class ClientBriefDetail extends React.Component {
  ...
  onRecommendSubmitAction = data => {
    const { data: body } = data;
    const apiCallData = {
      method: 'POST',
      body,
    };
    const requestURL = `${API_URL}${VERSION2}${JOB_POST}${TALENT}${STATUS}`;
    request(requestURL, apiCallData)
      .then(res => {
        this.loadBriefData();
        return this.onSubmitSuccess(res);
      })
      .catch(() => {
  ...
  setBriefData = response => {
    if (get(response, 'status')) {
      ...
      let ratePerHour = '';
      const employmentType = get(data, 'employmentType', []);
      ...
      if (employmentType.length > 0) {
        ...
        if (employmentType.includes('permanent-employee')) {
          const currencySymbol = getCurrencySymbol(currencyData, 'code', get(data, 'currencyAnnualRate'));
          ratePerHour = `${currencySymbol || ''}${get(data, 'annualRate')}`;
        }
      } else {
        const currencySymbol = getCurrencySymbol(currencyData, 'code', get(data, 'currency'));
        ratePerHour = `${currencySymbol || ''}${get(data, 'ratePerHour')}`;
      ...
      if (isAdmin) {
        ...
        briefData.projectStatus = projects.length > 0 && projects[0].status;
        projectName = (
      ...
      this.setState({
        ...
        briefData,
        isListLoading: false,
        recommendedTalents: get(data, 'recommendedTalents', []),
        talentDetails: get(data, 'talentDetails', []),
        interviewTalents: get(data, 'interviewTalents', []),
        shortlistedTalents: get(data, 'shortlistedTalents', []),
        hiredTalents: get(data, 'hiredTalents', []),
        rejectedTalents: get(data, 'rejectedTalents', []),
      });
    } else {
      toast.error(<ToastifyMessage message={get(response, 'message')} type="error" />, { className: 'Toast-error' });
    }
  ...
  renderTalentList = isListLoading => {
    ...
    const displayRecommendCount = 7;
    const recommendedTalentsShow = recommendedTalents.slice(0, displayRecommendCount);
    return (
      <React.Fragment>
        {!isListLoading && (
          <React.Fragment>
              ...
              <NavCard className="tab-sm pb-0 d-flex justify-content-between">
                <CustomNavTab tabs>
                  <NavItem>
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabRecommended} /> ({recommendedTalentsShow.length})
                    </CustomNavLink>
                  </NavItem>
                  <NavItem>
                    <CustomNavLink
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabApplications} /> ({talentDetails.length})
                    </CustomNavLink>
                  </NavItem>
                  <NavItem>
                    <CustomNavLink
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabInterviewed} /> ({interviewTalents.length})
                    </CustomNavLink>
                  </NavItem>
                  <NavItem>
                    <CustomNavLink
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabShortlisted} /> ({shortlistedTalents.length})
                    </CustomNavLink>
                  </NavItem>
                  <NavItem>
                    <CustomNavLink
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabHired} /> ({hiredTalents.length})
                    </CustomNavLink>
                  </NavItem>
                  <NavItem>
                    <CustomNavLink
                    ...
                    >
                      <FormattedMessage {...messages.hiringTabRejected} /> ({rejectedTalents.length})
                    </CustomNavLink>
                  </NavItem>
                </CustomNavTab>
              </NavCard>
              <TabContent activeTab={activeTab} className="mt-3">
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="1">
                  <div className="row">
                    {recommendedTalentsShow.length ? (
                      recommendedTalentsShow.map(cardItem => (
                        <Col className="col-xxl-4 col-lg-6 col-12 d-flex flex-column" key={get(cardItem, '_id', '')}>
                          <CardComponent
                            talentData={cardItem}
                            userRole={userRole}
                            briefTab="application"
                            project={project}
                            onSubmitAction={this.onRecommendSubmitAction}
                          />
                        </Col>
                      ))
                    ) : (
                      <CardEmpty>
                        <AppIcon.Empty src={ICONS.JobEmptyIcon} width="40" height="40" className="mb-3" />
                        <P className="p20 mb-2" opacityVal="0.5">
                          <FormattedMessage {...messages.notReceived} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
                </TabPane>
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="2">
                  <div className="row">
                    {talentDetails.length ? (
                      ...
                      <CardEmpty>
                        ...
                        <P className="p16 mb-0" opacityVal="0.5">
                          <FormattedMessage {...messages.noApplicants} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
                </TabPane>
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="3">
                  <div className="row">
                    {interviewTalents.length ? (
                      ...
                      <CardEmpty>
                        ...
                        <P className="p16 mb-0" opacityVal="0.5">
                          <FormattedMessage {...messages.noInterviewed} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
                </TabPane>
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="4">
                  <div className="row">
                    {shortlistedTalents.length ? (
                      shortlistedTalents.map(cardItem => (
                        <Col className="col-xxl-4 col-lg-6 col-12 d-flex flex-column" key={get(cardItem, '_id', '')}>
                          <CardComponent
                            talentData={cardItem}
                            userRole={userRole}
                            briefTab="shortlisted"
                            project={project}
                            onSubmitAction={this.onSubmitAction}
                          />
                        </Col>
                      ...
                      <CardEmpty>
                        ...
                        <P className="p16 mb-0" opacityVal="0.5">
                          <FormattedMessage {...messages.noShortListed} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
                </TabPane>
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="5">
                  <div className="row">
                    {hiredTalents.length ? (
                      ...
                      <CardEmpty>
                        ...
                        <P className="p16 mb-0" opacityVal="0.5">
                          <FormattedMessage {...messages.noHired} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
                </TabPane>
                <TabPane className="col-md-12 col-xl-12 p-0" tabId="6">
                  <div className="row">
                    {rejectedTalents.length ? (
                      rejectedTalents.map(cardItem => (
                        <Col className="col-xxl-4 col-lg-6 col-12 d-flex flex-column" key={get(cardItem, '_id', '')}>
                          <CardComponent
                            talentData={cardItem}
                            userRole={userRole}
                      ...
                      <CardEmpty>
                        ...
                        <P className="p16 mb-0" opacityVal="0.5">
                          <FormattedMessage {...messages.noRejected} />
                        </P>
                      </CardEmpty>
                    )}
                  </div>
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <div className="d-flex align-items-center">
                {!get(briefData, 'isArchived') ? (
                  <>
                    <div role="button" tabIndex={0} onClick={e => e.stopPropagation()} onKeyDown={() => {}}>
                      <ArchiveBriefModal
                        briefID={briefID}
                    ...
                    <div>
                      <AddBrief btnClassName="ms-3" type="edit" isAdmin={isAdmin} loadDetails={this.loadBriefData} {...briefEditData} />
                    </div>
                  </>
                ) : (
              ...
              {get(briefData, 'ratePerHour') && (
                <div className="d-flex">
                  ...
                  <P className="p16 mb-0 no-hover" opacityVal="0.5">
                    {get(briefData, 'employmentType').length > 0 ? (
                      <>{get(briefData, 'employmentType').includes('freelancer-consultant') ? '/hour' : '/annual'}</>
                    ) : (
                      '/hour'
                    )}
                  </P>
