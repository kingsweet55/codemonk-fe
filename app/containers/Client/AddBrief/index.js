...
import { getFilteredArrayObject } from 'containers/Auth/Preferences/utils';
...
export class AddBrief extends Component {
            ...
            {!showSearchTalentPopup ? (
              <div className="modal-body pt-0">
                ...
                <form onSubmit={handleSubmit}>
                  {currentStep === 1 && this.renderRoleForm()}
                  {currentStep === 2 && this.renderPreferredCandidateForm()}
                  {currentStep === 3 && this.renderEngagementForm()}
                </form>
              </div>
            ) : (
  ...
  handleAddBriefOpenModal = () => {
    ...
    const defaultEmpType = employmentTypeArray.find(i => i.value === 'freelancer-consultant') || [];
    const defaultCurrencyVal = { label: '$', value: 'USD' };
    ...
    if (type === 'add') {
      data = {
        ...
        briefRatePerHour: '',
        briefAnnualRate: '',
        briefCurrency: defaultCurrencyVal,
        briefAnnualCurrency: defaultCurrencyVal,
        briefLanguages: [languageData.find(i => i.code === 'en')] || [],
        briefContractType: defaultEmpType,
      };
      if (isProjectDetails) {
      ...
      const teamPreferenceObject = teamPreferenceArray.filter(o => teamPreference.includes(o.value));
      const employmentTypeObject =
        employmentType.length > 0 ? getFilteredArrayObject(employmentType, employmentTypeArray)[0] || [] : defaultEmpType;
      const workScheduleObject = getFilteredArrayObject(workPreference, workPreferenceArray) || [];
      const assignmentsObject = assignmentArray.filter(o => assignments.includes(o.value));
      ...
      const currencyFormater = v => currencyData.filter(o => o.code === v).map(obj => ({ label: obj.symbol, value: obj.code }))[0];
      ...
      data = {
        ...
        briefWorkSchedule: workScheduleObject,
        briefContractType: employmentTypeObject,
        ...
        briefDiscProfile: discProfile && formater(discProfile),
        briefTimeZone: formater(timeZone),
        briefCurrency: currency ? currencyFormater(currency) : defaultCurrencyVal,
        briefAnnualCurrency: currencyAnnualRate ? currencyFormater(currencyAnnualRate) : defaultCurrencyVal,
    ...
    this.setState({ addBriefFilters: filters, showBriefModal: true });
  };
  ...
  briefModalSuccess = () => {
    const { loadDetails, isAdmin, isProjectDetails = false, isClientHeader } = this.props;
    ...
    return (
      <React.Fragment>
        {type === 'add' ? (
          ...
          <ActionIcon type="button" className={btnClassName} onClick={this.handleAddBriefOpenModal}>
