...
import { API_URL, AGENCY, logoPlaceholder } from 'containers/App/constants';
...
export class AgencyDetailPage extends Component {
  ...
  renderAgencyLogo = agencyData => {
    const logoUrl = get(agencyData, 'tradingLogo');
    const agencyLogo = logoUrl ? <img src={logoUrl} alt="logo" /> : <SVG src={logoPlaceholder} />;
    return (
      <React.Fragment>
        ...
        <Row>
          ...
          <Col className="col-8">
            <LogoBlock>{agencyLogo}</LogoBlock>
          </Col>
        </Row>
      </React.Fragment>
    );
