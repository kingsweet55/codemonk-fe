...
export class Referrals extends React.Component {
  ...
  setReferralDataSuccess = response => {
    ...
    docs.forEach(referralData => {
      ...
      const referrerProfilePicture = get(referralData.referrer, 'profilePicture', '');
      const refereeProfilePicture = get(referralData.referee, 'profilePicture', '');
      const referrerBtnProps = {
        redirectTo: `${ADMIN}/talent-profile/`,
        talentId: get(referralData.referrer, 'talentId', ''),
        redirectType: 'adminTalentList',
        talentName: `${get(referralData.referrer, 'firstName')} ${get(referralData.referrer, 'lastName')}`,
        extra: {},
        profilePicture: referrerProfilePicture,
      };
      const refereeBtnProps = {
        redirectTo: `${ADMIN}/talent-profile/`,
        talentId: get(referralData.referee, 'talentId', ''),
        redirectType: 'adminTalentList',
        talentName: `${get(referralData.referee, 'firstName')} ${get(referralData.referee, 'lastName')}`,
        extra: {},
        profilePicture: refereeProfilePicture,
      };
      const referrer = <TalentNameButton {...referrerBtnProps} />;
      const referral = <TalentNameButton {...refereeBtnProps} />;
      const referralObj = {
        referrer,
        referral: get(referralData.referee, 'firstName') ? referral : '',
              ...
              <Card className="input-sm d-flex table-header border-0 p-0">
                <div className="d-flex ms-auto">
                  <FormGroup className="mb-3 mb-lg-0 me-3">
                    <FormLabel className="p14 mb-0">Referrer</FormLabel>
                    <Field
                      name="talentId"
                      type="text"
