...
describe('test functions', () => {
  ...
  test('Testing if setRecruiterProxyData with status 1', () => {
    const setRecruiterProxyData = jest.spyOn(getInstance(), 'setRecruiterProxyData');
    const data = {
      status: 1,
      data: {
        _id: '5f2a98a28bb50707f2d0b239',
    ...
    expect(setRecruiterProxyData).toHaveBeenCalledTimes(1);
  });

  test('Testing if setRecruiterProxyData with status 0', () => {
    const setRecruiterProxyData = jest.spyOn(getInstance(), 'setRecruiterProxyData');
    const data = {
      status: 0,
      data: {},
