...
export class ProxyLogin extends Component {
  ...
  setProxyData = response => {
    ...
    if (get(response, 'status')) {
      ...
      else if (type === 'talent-partner') this.setRecruiterProxyData(data);
      else if (type === 'ambassador') this.setAmbassadorProxyData(data);
    } else {
      toast.error(<ToastifyMessage message={get(response, 'message')} type="error" />, { className: 'Toast-error' });
    }
  };

  setRecruiterProxyData = data => {
    const currentSignupStep = get(data, 'signupStep', '') === 0.1 ? 0 : get(data, 'signupStep', 0);
    storeApiSignupStep(currentSignupStep);
    const signupStep = currentSignupStep;

    StorageService.set('signupStep', signupStep, { hash: true });
    StorageService.set('proxyType', 'talent-partner', { hash: true });
    StorageService.set('recruiterId', get(data, '_id'), { hash: true });
    StorageService.set('proxyToken', get(data, 'recruiterToken'), { hash: true });

    Emitter.emit(Emitter.EVENTS.Proxy_Login_Recruiter, true);
    const redirectPage = recruiterRedirectPageURL(signupStep);
    history.replace(redirectPage);
  };

  setAmbassadorProxyData = data => {
    const currentSignupStep = get(data, 'signupStep', '') === 0.1 ? 0 : get(data, 'signupStep', 0);
    storeApiSignupStep(currentSignupStep);
    const signupStep = currentSignupStep;

    StorageService.set('signupStep', signupStep, { hash: true });
    StorageService.set('proxyType', 'ambassador', { hash: true });
    StorageService.set('ambassadorId', get(data, '_id'), { hash: true });
    StorageService.set('proxyToken', get(data, 'ambassadorToken'), { hash: true });

    Emitter.emit(Emitter.EVENTS.Proxy_Login_Ambassador, true);
    const redirectPage = ambassadorRedirectPageURL(signupStep);
    history.replace(redirectPage);
  };
