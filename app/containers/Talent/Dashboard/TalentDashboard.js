...
import { SORT_TYPE } from '../../TalentListingPage/constants';
...
export class TalentDashboard extends React.Component {
  ...
  loadJobBriefs = () => {
    const data = { method: 'GET' };
    const requestURL = `${API_URL}${JOB_POST}${LIST}?${sortUrl(SORT_TYPE.recommend.value)}`;
    request(requestURL, data)
  ...
  setJobBriefs = response => {
    if (get(response, 'status')) {
      this.setState({ jobList: response.data.docs, totalJobCount: response.data.totalDocs, isJobCardsLoading: false });
    } else {
      toast.error(<ToastifyMessage message={get(response, 'message')} type="error" />, { className: 'Toast-error' });
    }
  };
  ...
  render() {
    const { roleType, projectList, userDetails, inviteLink, totalJobCount } = this.state;
    ...
    const lastName = get(userDetails, 'data.lastName');
    const updatedLastName = lastName ? `${lastName.charAt(0)}.` : '';
    return (
      <React.Fragment>
        <Helmet>
                ...
                <div>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <div className="d-flex">
                      ...
                      <Badge className="badge-sm ms-2 primary">{totalJobCount}</Badge>
                    </div>
                    <LinkViewAll
                      to={{
                ...
                <Card>
                  ...
                  <H3 className="mb-1">
                    {get(userDetails, 'data.firstName') ? (
                      <>
                        {get(userDetails, 'data.firstName')} {updatedLastName}
                      </>
                    ) : (
                      <>&nbsp;</>
                    )}
