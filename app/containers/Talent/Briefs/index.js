...
import { PrivateGrid, H3, H6, P, Badge, ToastifyMessage } from 'components';
...
import { SORT_BRIEFS } from '../../TalentListingPage/constants';
import { sortUrl } from '../../TalentListingPage/utils';
...
export class Briefs extends React.Component {
  constructor(props) {
    ...
    const talentBriefSort = JSON.parse(StorageService.get('talentBriefSort'));
    ...
    this.state = {
      ...
      totalCount: 0,
      pageSize: 8,
      currentSort: (talentBriefSort && SORT_BRIEFS.find(item => item.value === talentBriefSort)) || SORT_BRIEFS[0],
    };
  }
  ...
  loadTalentBrief = pageNum => {
    ...
    const { filters, search, pageSize, currentSort } = this.state;
    ...
    // sort
    requestURL += sortUrl(currentSort.value);
  ...
  handleSortChange = e => {
    StorageService.set('talentBriefSort', JSON.stringify(e.value));
              ...
              <Col lg={12}>
                <div className="d-flex justify-content-between mb-3">
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <SortSection>
                      <div className="sort-label ms-4">
                        <H6>{containerMessage.sortBy.defaultMessage}:</H6>
                      </div>
                      <SortComponent
                        sortArray={SORT_BRIEFS}
                        showSortIcon
