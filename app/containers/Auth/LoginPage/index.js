...
import { forgotPasswordLink, signupLink, recruiterSignupLink, ambassadorSignupLink } from 'containers/App/utils';
...
export class LoginPage extends Component {
  constructor(props) {
    ...
    this.state = { passwordType: 'password', userType, isAgency: false, isRecruiter: false, isAmbassador: false };
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    } else if (nextProps.location.pathname.split('/').includes('talent-partner')) {
      StorageService.set('selectedRoleType', 'recruiter', { hash: true });
      this.setState({ isRecruiter: true });
    } else if (nextProps.location.pathname.split('/').includes('ambassador')) {
      StorageService.set('selectedRoleType', 'ambassador', { hash: true });
      this.setState({ isAmbassador: true });
    } else {
  ...
  getSignUpLink = () => {
    ...
    let link = signupLink;
    if (isRecruiter) link = recruiterSignupLink;
    if (isAmbassador) link = ambassadorSignupLink;
    return link;
  };
                    ...
                    <div className="mt-4">
                      <>
                        ...
                        <P className="p16 mt-4">
                          ...
                          <A href={this.getSignUpLink()} className="ms-1 text-decoration-underline">
